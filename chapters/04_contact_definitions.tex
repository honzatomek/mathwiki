\newpage
\chapter{Contact Formulations}\label{chap:contact-formulations}
...
The same contact model can therefore be viewed at several levels. This chapter
uses the same three-way philosophy as Chapter~\ref{chap:multifreedom-constraints},
but contact adds a fourth ingredient because its active state is not known in
advance:

\begin{center}
\textbf{engineering interface law}
$\longleftrightarrow$
\textbf{discrete geometry}
$\longleftrightarrow$
\textbf{constraint enforcement}
$\longleftrightarrow$
\textbf{nonlinear algorithm}.
\end{center}

The engineering law answers whether the interface may open, slide, stick or
transmit tension. The geometric discretization decides where the gap and slip
are evaluated. The enforcement method decides how the resulting discrete
conditions enter the algebraic equations. Finally, the nonlinear algorithm has
to search for possible partners, update the contact state and repeat the
process as the bodies move.

\begin{figure}[ht]
    \centering
    \includegraphics[width=0.84\textwidth]{img/contact_formulation_layers.png}
    \caption{Four layers of a contact formulation. Settings from different
    layers are often presented together in finite element software, although
    they answer fundamentally different questions.}
    \label{fig:contact-formulation-layers}
\end{figure}

\begin{bbox}
\textbf{Python examples in this chapter.}

The code examples are executable counterparts of the derivations. As in
Chapter~\ref{chap:multifreedom-constraints}, \texttt{numpy} is used only for
array storage, indexing, elementary arithmetic and matrix products. No
\texttt{numpy.linalg}, sparse solver, eigensolver or other NumPy numerical
submodule is used. Small geometric operations and return mappings are written
explicitly so that the transition from equations to loops, state variables and
assembly operations remains visible.

The examples are intentionally small. Production contact solvers additionally
require spatial search trees, robust surface intersection algorithms, sparse
assembly, parallel data exchange and carefully linearized finite-sliding
kinematics.
\end{bbox}
...
Before introducing surface geometry, it is useful to make the state-dependent
logic explicit in the smallest possible program. The helper functions below are
also used by the later examples.
...
def vector_norm(v):
    """Euclidean norm without numpy.linalg."""
    return float(np.sqrt(v @ v))


def solve_2x2(A, b):
    """Explicit solution of a 2 x 2 system."""
    det = A[0, 0] * A[1, 1] - A[0, 1] * A[1, 0]
    if abs(det) < 1.0e-14:
        raise ValueError("singular 2 x 2 system")
    return np.array([
...
def solve_node_wall_contact(k, force, gap0):
    """Exact active-set solution of the 1D spring-wall problem."""
    u_free = force / k

    if u_free < gap0:
        return u_free, 0.0, "open"

    u = gap0
    pressure = force - k * u
    return u, pressure, "closed"


k = 1000.0
force = 1500.0
gap0 = 1.0
u, pressure, status = solve_node_wall_contact(k, force, gap0)
print(status, u, pressure)
# closed 1.0 500.0
\end{python}

This short function already contains an operation absent from an ordinary
linear finite element solve: a \textit{branch}. The code first solves the open
problem, checks whether the trial state violates the gap, and only then selects
the closed equations. With many contact points, this branch becomes an active
set whose membership changes during the nonlinear iterations.
...
\begin{figure}[ht]
    \centering
    \includegraphics[width=0.88\textwidth]{img/contact_state_machine.png}
    \caption{Contact as a state machine. Frictional contact adds a second status
    decision after the normal open/closed decision.}
    \label{fig:contact-state-machine}
\end{figure}
...
The transition from the continuum statement to a program begins with geometry.
A contact solver does not receive a matrix row directly; it receives nodes,
faces, current coordinates and search tolerances. It must determine which
surface points correspond to each other before a gap equation can be assembled.
The local normal/tangent contact frame is a position-dependent basis in exactly
the sense developed in Chapter~\ref{chap:coordinate-transformations}; contact
adds the additional requirement that this basis may have to be rebuilt as the
projection point and surface orientation evolve.

\begin{figure}[ht]
    \centering
    \includegraphics[width=0.66\textwidth]{img/contact_gap_projection.png}
    \caption{Closest-point projection used to construct a node-to-surface
    normal gap. The projection coordinates, surface normal and interpolation
    weights are geometric data that must be recomputed when the configuration
    changes.}
    \label{fig:contact-gap-projection}
\end{figure}
...
\subsection{From Surface Geometry to a Contact Row}

The node-to-surface MPC of Chapter~\ref{chap:multifreedom-constraints} provides
a useful starting point. If a slave point is projected to a target face and the
target interpolation at the projection is $N_i$, the small-sliding normal gap
may be written

\begin{equation}\label{contact-discrete-gap-row}
    g_n = \m{n}^T\left(
        \m{x}_s - \sum_{i=1}^{n_m} N_i \m{x}_i
    \right).
\end{equation}

For fixed $N_i$ and $\m{n}$, its displacement derivative is the row

\begin{equation}\label{contact-gap-B-row}
    \m{B}_n =
    \begin{bmatrix}
        \m{n}^T & -N_1\m{n}^T & -N_2\m{n}^T & \cdots & -N_{n_m}\m{n}^T
    \end{bmatrix}.
\end{equation}

This is the contact counterpart of a surface MPC row. The essential difference
is not the interpolation itself, but the law applied to the scalar quantity
$g_n$: a tied MPC enforces it as an equality at all times, whereas ordinary
contact activates the constraint only when the unilateral conditions require
it.

The following compact implementation constructs the geometry for a planar
Quad4 face. It performs the projection explicitly and returns both the normal
gap and the coefficients of $\m{B}_n$ together with their nodal DOF ordering.
...
def project_to_quad4(point, face_xyz, xi=0.0, eta=0.0,
                     tolerance=1.0e-12, max_iter=20):
    """Closest-point projection by a two-parameter Gauss-Newton iteration."""
    for _ in range(max_iter):
        ...
        rhs = -np.array([a1 @ residual, a2 @ residual])
        dxi = solve_2x2(G, rhs)
        xi += dxi[0]
        eta += dxi[1]

        if vector_norm(dxi) < tolerance:
            break

    return xi, eta


def build_node_to_surface_contact(slave_xyz, face_xyz):
    xi, eta = project_to_quad4(slave_xyz, face_xyz)
    N = quad4_shape(xi, eta)
    dN_dxi, dN_deta = quad4_shape_derivatives(xi, eta)
    ...
    gap = (slave_xyz - x_master) @ normal

    B = np.zeros(15)       # 3 slave + 4 x 3 master DOFs
    B[0:3] = normal
    for i in range(4):
        B[3 + 3*i: 6 + 3*i] = -N[i] * normal

    return gap, normal, N, B, (xi, eta)
...
slave = np.array([0.5, 0.25, -0.05])
gap, normal, N, Bn, local = build_node_to_surface_contact(slave, face)
print(local)
print(N)
print(gap)
# (-0.5, -0.5)
...
The output should be read as a chain of derived data, not merely as four
numbers. The node and face coordinates generate the projection coordinates;
those generate the shape functions and normal; those generate $g_n$ and
$\m{B}_n$; and only then can the contact force and tangent be assembled.

\begin{bbox}
\textbf{Engineering note -- projection is not only interpolation.}

The compact routine above assumes that the closest projection lies on the
interior of the Quad4 face. A production implementation must additionally test
parametric bounds, edges and vertices and may have to switch to a neighbouring
face. The coefficients of a contact row therefore depend on a geometric search
algorithm, not only on the element shape functions.
\end{bbox}

For finite sliding or deforming curved surfaces, $N_i$, $\m{n}$ and therefore
$\m{B}_n$ depend on the current solution. Their derivatives contribute to a
consistent contact tangent. The compact expression
$\m{B}_n=\partial g_n/\partial\m{u}$ therefore represents data that a real
program must repeatedly rebuild.
...
For a general discrete contact point, the same operation is a small matrix
assembly. If $g_n<0$ and geometric derivatives are temporarily neglected,

\begin{equation}
    \m{r}_c=-\m{B}_n^T p_n,
    \qquad
    \m{K}_c\approx\epsilon_n\m{B}_n^T\m{B}_n.
\end{equation}

The outer product is the same assembly pattern encountered in penalty MFCs in
Chapter~\ref{chap:multifreedom-constraints}.

\begin{python}
def penalty_contact_point(gap, Bn, epsilon):
    """Pressure, residual and small-sliding tangent for one contact point."""
    n = Bn.shape[0]
    residual = np.zeros(n)
    tangent = np.zeros((n, n))

    if gap >= 0.0:
        return 0.0, residual, tangent, "open"

    pressure = -epsilon * gap
    residual -= Bn * pressure

    for i in range(n):
        for j in range(n):
            tangent[i, j] += epsilon * Bn[i] * Bn[j]

    return pressure, residual, tangent, "closed"


pressure, rc, Kc, status = penalty_contact_point(gap, Bn, 1.0e5)
...
This small function is already recognizable as finite element assembly: test
the state, calculate a local force law, form a local tangent, and later scatter
those local contributions into the global residual and stiffness arrays.
...
The one-dimensional spring-wall problem makes the augmentation loop transparent.
Using penetration $d=u-g_0$ and a current pressure estimate $p^j$, an active
penalty subproblem is

\begin{equation}
    k u + p^j + \epsilon_n(u-g_0)=f,
\end{equation}

followed by

\begin{equation}
    p^{j+1}=\left\langle p^j+\epsilon_n(u-g_0)\right\rangle.
\end{equation}

\begin{python}
def augmented_node_wall(k, force, gap0, epsilon, n_updates=6):
    pressure = 0.0
    history = []

    # The open trial state must still be checked first.
    u_free = force / k
    if u_free < gap0:
        return [(u_free, 0.0, 0.0)]

    for _ in range(n_updates):
        # Active penalty subproblem: scalar equilibrium solved explicitly.
        u = (force - pressure + epsilon * gap0) / (k + epsilon)
        penetration = u - gap0
        pressure = max(0.0, pressure + epsilon * penetration)
        history.append((u, penetration, pressure))

    return history


for u, penetration, pressure in augmented_node_wall(
        1000.0, 1500.0, 1.0, 1000.0):
    print(u, penetration, pressure)
# 1.25      0.25      250.0
# 1.125     0.125     375.0
# 1.0625    0.0625    437.5
# ... -> u = 1.0, p = 500.0
...
The code makes the extra algorithmic level clear. A penalty method modifies one
solve; an augmented method additionally stores a history variable and wraps
repeated correction steps around that solve.
...
\begin{figure}[ht]
    \centering
    \includegraphics[width=0.48\textwidth]{img/contact_friction_limit.png}
    \caption{Tangential traction space for an isotropic Coulomb law. A trial
    traction inside the circle sticks; a trial traction outside the admissible
    region is returned to the slip boundary $\|\m{t}_t\|=\mu p_n$.}
    \label{fig:contact-friction-limit}
\end{figure}

The return mapping can be implemented without a nonlinear equation solver. The
algorithm stores the previous tangential traction, forms the elastic trial
state and branches according to the yield function.

\begin{python}
def coulomb_return(pressure, traction_old, slip_increment,
                   epsilon_t, mu):
    """Two-component tangential penalty predictor / Coulomb return."""
    if pressure <= 0.0:
        return np.zeros_like(traction_old), "open"

    trial = traction_old - epsilon_t * slip_increment
    trial_norm = vector_norm(trial)
    limit = mu * pressure
    ...
    traction = (limit / trial_norm) * trial
    return traction, "slip"


p_n = 1000.0
mu = 0.3
epsilon_t = 2.0e4
traction_old = np.array([100.0, 0.0])
delta_gt = np.array([-0.02, -0.01])
traction, state = coulomb_return(
    p_n, traction_old, delta_gt, epsilon_t, mu)
print(state, traction, vector_norm(traction))
# slip ... 300.0
\end{python}

The mathematical traction vector is therefore a program history variable. The
solver must preserve it from one converged increment to the next, rotate or
transport it when the local tangential basis changes, erase it when contact
opens, and update it differently for stick and slip. This is one reason why
frictional contact is substantially more than adding a shear spring between two
nodes.
...
Chapter~\ref{chap:multifreedom-constraints} derived node-to-surface and weak
surface-to-surface equations as permanent bilateral ties. Contact uses much of
the same search, projection and interpolation machinery, but applies a
state-dependent normal and tangential law to the resulting relative motion.

\begin{figure}[ht]
    \centering
    \includegraphics[width=0.91\textwidth]{img/contact_discretizations.png}
    \caption{Three levels of contact discretization. The physical contact law
    may be identical while the points at which it is enforced and the way
    tractions are transferred differ.}
    \label{fig:contact-discretizations}
\end{figure}
...
\subsection{From Contact Points to Global Residual and Tangent}

Regardless of whether the contact points are slave nodes, segment quadrature
points or mortar integration points, their contributions eventually have to be
assembled into global arrays. If contact point $q$ has quadrature weight $w_q$,
gap row $\m{B}_{nq}$ and pressure $p_{nq}$, the small-sliding penalty
contribution is schematically

\begin{equation}
    \m{r}_c = \sum_q -w_q\m{B}_{nq}^T p_{nq},
    \qquad
    \m{K}_c \approx \sum_{q\in\mathcal A}
        w_q\epsilon_n\m{B}_{nq}^T\m{B}_{nq}.
\end{equation}

A real program normally stores only the nonzero DOF indices of each
$\m{B}_{nq}$ and scatters the small contact block into sparse global storage.
The following dense version makes the loops explicit.

\begin{python}
def assemble_penalty_contact(n_dofs, contact_points, epsilon):
    """Assemble contact residual/tangent from already constructed points."""
    residual = np.zeros(n_dofs)
    tangent = np.zeros((n_dofs, n_dofs))
    ...
    for point in contact_points:
        ...
        for a, I in enumerate(dofs):
            residual[I] -= weight * B[a] * pressure
            for b, J in enumerate(dofs):
                tangent[I, J] += (
                    weight * epsilon * B[a] * B[b]
                )

    return residual, tangent


# Two scalar demonstration contact points in a 3-DOF model.
points = [
    {"gap": -0.01, "dofs": [0, 1],
     "B": np.array([1.0, -1.0]), "weight": 0.5},
    {"gap":  0.02, "dofs": [1, 2],
     "B": np.array([1.0, -1.0]), "weight": 0.5},
]
rc, Kc = assemble_penalty_contact(3, points, 1.0e5)
print(rc)
print(Kc)
\end{python}

The second point contributes nothing because it is open. The branch inside the
quadrature loop is therefore part of the matrix assembly itself. As the active
set changes, the sparsity pattern and values of the contact tangent can change
...
\begin{figure}[ht]
    \centering
    \includegraphics[width=0.93\textwidth]{img/contact_algorithm_pipeline.png}
    \caption{Discrete contact operations within a nonlinear iteration. The
    displacement correction changes the geometry, so the geometric and state
    data cannot generally be constructed once and reused unchanged.}
    \label{fig:contact-algorithm-pipeline}
\end{figure}
...
\subsection{What a Contact Point Stores}

The continuum notation often gives the impression that a contact surface is one
single mathematical object. A program instead manages a collection of discrete
contact records. A typical record may contain

\begin{center}
\texttt{slave entity, target face, local coordinates, normal, gap, pressure,}\
\texttt{tangential traction, stick/slip state, previous projection and history.}
\end{center}

Some entries are geometry, some are unknown-like state variables, and some are
history variables. During finite sliding the target face itself may change. A
contact algorithm is therefore partly a mechanics algorithm and partly a data
management algorithm.

A stripped-down status update illustrates this structure:

\begin{python}
def update_contact_record(record, epsilon_n, epsilon_t, mu):
    gap = record["gap"]

    if gap > 0.0:
        record["pressure"] = 0.0
        record["traction_t"][:] = 0.0
        record["status"] = "open"
        return record

    pressure = max(0.0, -epsilon_n * gap)
    traction, state = coulomb_return(
        pressure,
        record["traction_t"],
        record["delta_gt"],
    ...
    record["traction_t"][:] = traction
    record["status"] = state
    return record
\end{python}

This function is not a complete contact solver, but it exposes a fundamental
implementation fact: open/closed/stick/slip are not annotations added after the
solution. They are state variables that decide which local equations are
assembled into the next global solve.
...
\begin{notebooknote}[Notebook note -- Search distance is geometry bookkeeping, not contact physics]
This distinction is worth keeping explicit because search settings and penalty
settings often sit next to each other in a preprocessor. Increasing the candidate
search distance may find more possible pairs, but it does not make contact
``stiffer''. Conversely, increasing normal penalty stiffness cannot recover a
pair that the geometric search never considered.
\end{notebooknote}
...
The programming distinction is equally important. In Chapter~\ref{chap:multifreedom-constraints},
a linear MFC row can usually be constructed during preprocessing and reused.
In contact, the corresponding row may depend on a projection point, a current
normal and a current active state. The solver may therefore have to search,
rebuild and reclassify it repeatedly. The matrix equation is only one snapshot
of an evolving discrete algorithm.
...
The next chapter returns to permanent bilateral relations. There, the same
separation between physical constraint definition and numerical enforcement is
used to organize rigid elements, interpolation couplings, fixed-distance
constraints and remote coupling objects.
